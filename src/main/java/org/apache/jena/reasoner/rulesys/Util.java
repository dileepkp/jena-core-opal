...
public class Util {
    ...
    public static Node getSubjValue( Node prop, Node value,RuleContext context) {
        ClosableIterator<Triple> it=context.find(null, prop, value);
         Node result = null;
        if (it.hasNext()) {
    ...
    /**
     * Helper - returns the all values for the given property on the given
     * root node in the data graph.
     */
    public static List<Node> getPropValueList(Node root, Node prop, RuleContext context) {
        //return doGetPropValue(context.find(root, prop, null));
        ClosableIterator<Triple> list = context.find(root, prop, null);
        List<Node> objectList= new ArrayList<>();
        Node temp = null;
        while(list.hasNext()){
            temp = list.next().getObject();
            objectList.add(temp);
        }
        return objectList;
    }
    
    public static List<Node> getSubjValueList( Node prop, Node value,RuleContext context) {
        //return doGetPropValue(context.find(root, prop, null));
        ClosableIterator<Triple> list = context.find(null, prop, value);
        List<Node> subjectList= new ArrayList<>();
        Node temp = null;
        while(list.hasNext()){
            temp = list.next().getSubject();
     ...
     /**
     * Helper - returns a value for the given property on the given
     * root node in the data graph based on join condition.
     */
    public static Node getJoinPropValue(Node root, Node prop, Node prop2, Node value2, RuleContext context) {
        //return doGetPropValue(context.find(root, prop, null));
        ClosableIterator<Triple> list = context.find(root, prop, null);
        //List<Node> subjectList= new ArrayList<>();
        Node temp = null;
        while(list.hasNext()){
            temp = list.next().getObject();
            if(context.contains(temp,prop2,value2))
                return temp;
        }
        return null;
    }
     ...
     /**
     * Helper - returns a subject value for the given properties and values
     * in the data graph based on join condition. fork like queries.
     */
    public static Node getJoinSubjValue(Node prop, Node value, Node prop2, Node value2, RuleContext context) {
        //return doGetPropValue(context.find(root, prop, null));
        ClosableIterator<Triple> list = context.find(null, prop, value);
        //List<Node> subjectList= new ArrayList<>();
        Node temp = null;
    ...
    /**
     * Queries of type ?s(output) p1 o1. o1 p2 o2.
     * Start with second as o1 is unique
     */
    public static List<Node> getSubjValueJoinOnObj(Node prop, Node prop2, Node value2, RuleContext context) {
        //List<Node> subjectList= new ArrayList<>();
        Node value = getSubjValue(prop2, value2, context);
        return getSubjValueList(prop, value, context);
    }
    ...
    public static List<Node> convertList(Node root, RuleContext context) {
        ...
        return convertListTemp(root, context, new LinkedList<Node>());
    }
    ...
    //added by dileep for some testing purpose
     private static List<Node> convertListTemp( Node node, RuleContext context, List<Node> sofar ) {
        if (node == null || node.equals(RDF.nil.asNode())) return sofar;
        for(int i=0; i< 100; ++i){
            Node next = getPropValue(node, RDF.first.asNode(), context);
            if (next == null || node.equals(RDF.nil.asNode())) {
                return sofar;
            } else {
                sofar.add(next);
                //return convertList(getPropValue(node, RDF.rest.asNode(), context), context, sofar);
    ...
    public static Node makeStringNode(String value) {
        return NodeFactory.createLiteral(LiteralLabelFactory.createTypedLiteral( value ));
    }
    
    public static Node makeURINode(String value) {
