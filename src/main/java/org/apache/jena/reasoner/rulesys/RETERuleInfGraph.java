...
public class RETERuleInfGraph extends BasicForwardRuleInfGraph {
    ...
    public synchronized void performAdd(Triple t) {
      ...
      /**
     * Add many triples to the data graph, run any rules triggered by
     * the new data items, recursively adding any generated triples.
     */
    public synchronized void performAdd(Graph triples) {
        this.requirePrepared();
        //FGraph newfdata = new FGraph(triples.);
        FGraph newfdata = new FGraph(triples);
        
         for(Triple t: ((RETEEngine)engine).deletedTriples){
            newfdata.getGraph().add(t);
            ((RETEEngine)engine).addTriple(t,true);
        }
        
        for(Triple t: triples.find().toList()){
            newfdata.getGraph().add(t);
            ((RETEEngine)engine).addTriple(t,false);
        }
       
      
       //engine.fastInit(newfdata);
       ((RETEEngine)engine).deletedTriples.clear();
       ((RETEEngine)engine).runAll();
        
    }
    
    public void performDelete(Graph triples) {
