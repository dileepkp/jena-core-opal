...
public class ModelFactory extends ModelFactoryBase
{
    ...
    */
    ...
    public static InfModel createInfModel( Reasoner reasoner, Model model,List <Statement> topLevelStatments ) {
        //GenericRuleReasoner reasoner2 = (GenericRuleReasoner) reasoner;
         RETERuleInfGraph graph = (RETERuleInfGraph)reasoner.bind(model.getGraph());
         //Graph g = new GraphMem();
         for(Statement s : topLevelStatments){
           //  g.clear();
           //  g.add(s.asTriple());
           //  graph = reasoner2.bind2((RETERuleInfGraph)graph, g);
           graph.performAdd(s.asTriple());
         }
         return new InfModelImpl(graph);
    }
